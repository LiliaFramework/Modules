...
function PANEL:Init()
    ...
    self.right.Paint = function()
        ...
        local currentWeight = inv:getWeight()
        local maxWeight = inv:getMaxWeight()
        ...
        if lia.class.list[class] then
            ...
            draw.DrawText(L("weight"), "liaSmallFont", sw * 0.085, sh * 0.13, color_white, TEXT_ALIGN_LEFT)
            draw.DrawText(currentWeight .. "/" .. maxWeight .. lia.config.get("invWeightUnit", "KG"), "liaSmallFont", sw * 0.2, sh * 0.13, color_white, TEXT_ALIGN_RIGHT)
        else
            ...
            draw.DrawText(L("weight"), "liaSmallFont", sw * 0.085, sh * 0.11, color_white, TEXT_ALIGN_LEFT)
            draw.DrawText(currentWeight .. "/" .. maxWeight .. lia.config.get("invWeightUnit", "KG"), "liaSmallFont", sw * 0.2, sh * 0.11, color_white, TEXT_ALIGN_RIGHT)
        end
    end
...
function PANEL:setItemType(itemType)
    ...
    self.name:SetTextColor(RarityColors[rarity] or color_white)
end
...
function PANEL:updateAction()
    ...
    self.action.DoClick = function()
        if self.isSelling then
            if IsValid(lia.gui.vendor) then lia.gui.vendor:sellItemToVendor(self.item.uniqueID) end
        else
            if IsValid(lia.gui.vendor) then lia.gui.vendor:buyItemFromVendor(self.item.uniqueID) end
        end
