...
PANEL = {}
function PANEL:Init()
    if not LocalPlayer():hasPrivilege("useCinematicMenu") then return end
    ...
    bigTextLabel:SetText("Large Splash Text")
    ...
    quitButton:SetText(string.upper("cancel"))
    ...
    postButton:SetText(string.upper("Post"))
    ...
    postButton.DoClick = function()
        if not (self.contents.text or self.contents.bigText) then
            lia.notices.notify("Reopen the panel and try again.")
            return
        end

        if self.contents.text == "" and self.contents.bigText == "" then
            lia.notices.notify("You must enter some text.")
            return
        end
