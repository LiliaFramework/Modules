...
if SERVER then
    ...
    concommand.Add("mcompass_spot", function(ply, _, args)
        ...
        if tr.Entity and not tr.HitWorld then
            ...
            MODULE.AddMarker(ply, tr.HitPos, nil, t, color)
        end
    end)
else
    ...
    hook.Add("PopulateToolMenu", "mCompass_PopulateToolMenu", function()
        spawnmenu.AddToolMenuOption("Options", "mCompass", "Settings", "Settings", "", "", function(panel)
            ...
            Label1:SetText(L("compassClientSettings"))
            Label1:SizeToContents()
            panel:AddItem(Label1)
            local box = vgui.Create("DCheckBoxLabel", panel)
            box:SetText(L("compassEnabled"))
            ...
            box2:SetText(L("compassHeading"))
            box2:SetTextColor(Color(50, 50, 50))
            box2:SetConVar("mcompass_heading")
            box2:SetValue(GetConVar("mcompass_heading"):GetInt())
            box2:SizeToContents()
            panel:AddItem(box2)
            panel:NumSlider(L("compassStyle"), "mcompass_style", 1, 3, 0)
            panel:NumSlider(L("compassXPosition"), "mcompass_xposition", 0, 1)
            panel:NumSlider(L("compassYPosition"), "mcompass_yposition", 0, 1)
            panel:NumSlider(L("compassWidth"), "mcompass_width", 0, 1)
            panel:NumSlider(L("compassHeight"), "mcompass_height", 0, 1)
            panel:NumSlider(L("compassSpacing"), "mcompass_spacing", 1, 10, 2)
            panel:NumSlider(L("compassRatio"), "mcompass_ratio", 0.1, 10)
            ...
            but1:SetText(L("compassSetColor"))
            ...
            but2:SetText(L("compassResetSettings"))
