...
function MODULE:HUDPaint()
    ...
    for i = #damageNumbers, 1, -1 do
        ...
        if timeLeft <= 0 then
            ...
            if scr.visible then
                draw.SimpleTextOutlined(dn.text, lia.config.get("DamageFont", "Arial"), scr.x, scr.y - (255 - alpha) / 2, ColorAlpha(dn.color, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, alpha))
            else
                table.remove(damageNumbers, i)
            end
        end
