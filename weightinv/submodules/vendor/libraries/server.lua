...
function MODULE:VendorTradeEvent(client, vendor, itemType, isSellingToVendor)
    if not VENDOR_INVENTORY_MEASURE and lia.inventory.types["WeightInv"] then
        VENDOR_INVENTORY_MEASURE = lia.inventory.types["WeightInv"]:new()
        VENDOR_INVENTORY_MEASURE.virtual = true
        VENDOR_INVENTORY_MEASURE:setData("maxWeight", lia.config.get("invMaxWeight", 150))
    ...
    if isSellingToVendor then
        ...
        if item then
            ...
            item:remove():next(function()
                ...
                client.vendorTransaction = nil
            end):catch(function() client.vendorTransaction = nil end)
        else
            client.vendorTransaction = nil
        end
    else
        local canFit = inventory:canItemFitInInventory(lia.item.list[itemType])
        if not canFit then
        ...
        inventory:add(itemType):next(function(item)
