...
function MODULE:OnPlayerHitGround(client, inWater, onFloater, speed)
	if (client.rappelling && client.rappelPos.z - client:GetPos().z > 64) then
		self:EndRappel(client)
		...
		if (speed >= 196) then
			client:ViewPunch(viewPunchAngle)
		end
	end
end
...
function MODULE:FinishMove(client, moveData)
	if (client:HasWeapon("lia_rappel_gear")) then
		...
		if (onGround && !client.wasOnGround) then
			client.wasOnGround = true
		elseif (!onGround && client.wasOnGround) then
			client.wasOnGround = false

			if (!client.rappelling && moveData:KeyDown(IN_WALK) && client:GetMoveType() != MOVETYPE_NOCLIP) then
...
function MODULE:Move(client, moveData)
	if (client.rappelling) then
		...
		if (!client:OnGround() && (client:EyePos().z) < client.rappelPos.z) then
			rappelFalling = true
