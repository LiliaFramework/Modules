...
function PANEL:Init()
    ...
    self.right.DoClick = function()
        ...
        self.slides[self.currentSlide]:MoveTo(-ScrW(), (ScrH() * .5) - (ScrH() * .7) / 2, .3, 0, -1, function(_, me) if me.Hidden then me:Hidden() end end)
        ...
        pnl.Setup = function()
            ...
            function pnl.models:Setup()
                ...
                for k, v in SortedPairs(faction.models) do
                    ...
                    icon.index = k
                    if realpanel.context:GetModel() == k then self:onModelSelected(icon, true) end
                end
    ...
    do
        ...
        pnl.Setup = function()
            ...
            pnl.finish.DoClick = function()
                ...
                liaMultiChar:createCharacter(self.context):next(
                    function()
                        realpanel:Hide()
                        LocalPlayer():EmitSound(unpack(SOUND_CHAR_CLICK))
                    end,
                    function() LocalPlayer():EmitSound(unpack(SOUND_CHAR_WARNING)) end
                )
            end
        end
