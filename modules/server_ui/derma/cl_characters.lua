...
function PANEL:Init()
    if IsValid(lia.gui.newCharsMenu) then lia.gui.newCharsMenu:Remove() end
...
function PANEL:onCharacterSelected(character)
    ...
    self.choosing = true
    self:setFadeToBlack(true):next(function() return liaMultiChar:chooseCharacter(character:getID()) end):next(
        function(err)
            self.choosing = false
