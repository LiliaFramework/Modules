...
function PANEL:Init()
	...
	function self.close.Think(this)
		if self.wp and IsValid(self.wp) then
			this:SetZPos(self.wp:GetZPos() + 1)
		end
	end
	...
	function self:Think()
		if self.blurIncrease and self.blurIncrease:Active() then
			self.blurIncrease:Run()
		end

		if self.blurDecrease and self.blurDecrease:Active() then
			self.blurDecrease:Run()
		end

		if self.camMover and self.camMover:Active() then
			self.camMover:Run()
		end
	...
	MODULE:Intro(
		...
		function()
			...
			title.logoPnl:AlphaTo(
				255,
				0.8,
				delay,
				function()
					title.buttonScroll:AlphaTo(255, 0.5, 0)
				end
			)
...
function PANEL:OnKeyCodePressed(key)
	if key == KEY_F1 and not self.comesFromF1 then
		self:Remove()
	end
