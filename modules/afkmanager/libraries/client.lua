...
local function DrawWarning()
    ...
    draw.DrawText(lia.config.get("AFKWarningHead", "WARNING!"), "AFKKicker120", ScrW() * 0.5, (ScrH() * 0.5) - ScreenScale(50), Color(255, 0, 0, MODULE.Alpha), TEXT_ALIGN_CENTER)
    draw.DrawText(lia.config.get("AFKWarningSub", "You are going to be sent back to the character menu/kicked for being AFK!\nPress any key to abort!") .. "\nYou will be kicked in " .. math.floor(math.max(lia.config.get("AFKKickTime", 30) - (CurTime() - MODULE.WarningStart), 0)) .. "s", "AFKKicker25", ScrW() * 0.5, ScrH() * 0.5, Color(255, 255, 255, MODULE.Alpha), TEXT_ALIGN_CENTER)
end
