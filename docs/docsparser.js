...
if (!fs.existsSync(modulesTxtPath)) { console.log("No modules.txt found, exiting docs build."); process.exit(0); }
...
for (let i = 0; i < lines.length; i += 3) {
    ...
    data.push({ name: escapeHTML(nameLine), author: escapeHTML(authorLine), desc: escapeHTML(descLine) });
}
...
`;
    ...
    if (found) {
        ...
        const moduleZipDest = path.join(downloadsDir, found);
        fs.copyFileSync(moduleZipSrc, moduleZipDest);
        html += `  <p><a href="Downloads/${found}" download>Download Module ZIP</a></p>\n`;
    } else {
